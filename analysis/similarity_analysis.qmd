...
clip_plots <- generate_multimodal_plots(clip_data_summarized |> filter(vanilla_trial==1), "CLIP")
clip_plots
```
...
Multimodal model did not converge, trying to see if there are missing values.
```{r}
...
getting rid of singular effects and lack of convergence
```{r}
pruned_image_model <- lmer(scale(mean_target_looking_critical_window) ~ scale(image_similarity)*scale(age)
...
pruned_multimodal_model <- lmer(scale(mean_target_looking_critical_window) ~ scale(multimodal_similarity)*scale(age)
                    ...
                    + (1|dataset_id), data = model_data, control = lmerControl(optimizer ="Nelder_Mead"))

pruned_ooo_model <- lmer(scale(mean_target_looking_critical_window) ~ scale(ooo_similarity)*scale(age)
                    + scale(aoa) + scale(MeanSaliencyDiff)
                    + (1 | subject_id)
                    + (1 | text1:unique_pair)
                    + (1|dataset_id), data = usable_trials_summarized_with_sims)
summary(pruned_image_model)
summary(pruned_text_model)
summary(pruned_multimodal_model)
summary(pruned_ooo_model)
```
...
## LWL specific ages and AoA? Under 3 year olds
```{r}
