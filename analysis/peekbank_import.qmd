...
stimuli_trial_info <-  downloaded_stimuli |> 
      select(stimulus_id, image_description, local_stimulus_path, english_stimulus_label) |>
      mutate(english_stimulus_label = str_to_lower(english_stimulus_label))
...
unique_stimuli_pairs <- downloaded_stimuli_trials |>
  ...
  rename(text1=english_stimulus_label_target, text2=english_stimulus_label_distractor, text1_description=image_description_target, text2_description=image_description_distractor, image1=local_stimulus_path_target, image2=local_stimulus_path_distractor,dataset_name=dataset_name)
...
usable_trials_summarized_with_sims <- usable_trials_summarized |> select(-dataset_id, -dataset_name) |>
  ...
  left_join(similarities |> select(-trial_type_id, -target_id, -distractor_id), by = c("unique_pair")) |>
...
cols_to_update <- colnames(similarities|> select(-trial_type_id, -target_id, -distractor_id)) 
usable_trials_imagepair_sims <- usable_trials_summarized_with_sims |>
  mutate(unique_imagepair = pmap_chr(list(local_stimulus_path_target, local_stimulus_path_distractor), ~ paste(sort(c(.x, .y)), collapse = "_")),
         unique_textpair = pmap_chr(list(text1, text2), ~ paste(sort(c(.x, .y)), collapse = "_"))) |>
...
# moving this to later in processing for now 
...
print(paste("Number of unique label-target-distractor pairs covered by THINGS:", count(usable_trials_summarized_with_sims |> filter(!is.na(ooo_similarity)) |> distinct(image_description_target, image_description_distractor, dataset_name))))
